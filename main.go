...
func newS3Collector(cephAccessKey, cephSecretKey, cephGatewayURL string) *S3Collector {
	return &S3Collector{
		...
		totalUsageSizeActualMetric: prometheus.NewDesc("s3_bucket_total_usage_size",
		...
		perUserUsageMetric: prometheus.NewDesc("s3_bucket_per_user_usage_size",
			"size of each user",
			[]string{"name"}, nil,
		),
		bucketQuotaMaxSize: prometheus.NewDesc("s3_bucket_quota_max_size",
			"bucket quota max size",
			[]string{"name"}, nil,
		),
		bucketQuotaMaxObjects: prometheus.NewDesc("s3_bucket_quota_max_objects",
			"bucket quota max objects",
			[]string{"name"}, nil,
		),
		bucketNumShards: prometheus.NewDesc("s3_bucket_num_shards",
